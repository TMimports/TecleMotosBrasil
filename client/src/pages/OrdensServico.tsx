...
export function OrdensServico() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const partesChassi: string[] = [];
      if (form.motoChassi.trim()) partesChassi.push(`Chassi: ${form.motoChassi.trim()}`);
      if (form.motoMotor.trim()) partesChassi.push(`Motor: ${form.motoMotor.trim()}`);
      const motoDescricaoFinal = [form.motoDescricao.trim(), ...partesChassi].filter(Boolean).join(' | ');
      ...
      const novaOs = await api.post<OrdemServicoFull>('/os', {
        ...
        motoDescricao: motoDescricaoFinal,
