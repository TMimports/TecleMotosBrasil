...
export function OrdensServico() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const motoDescricaoCompleta = [
        form.motoDescricao || '',
        form.chassi ? `Chassi: ${form.chassi}` : '',
      ...
      const novaOs = await api.post<OrdemServicoFull>('/os', {
        ...
        motoDescricao: motoDescricaoCompleta,
      ...
      setModalOpen(false);
      setForm({ clienteId: '', lojaId: lojas.length === 1 ? String(lojas[0].id) : '', tecnicoId: '', motoDescricao: '', chassi: '', motor: '', observacoes: '', tipo: 'OS', desconto: '0', descontoServico: '0' });
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              ...
              <button type="button" onClick={() => setShowQuickCliente(true)} className="text-xs text-orange-400 hover:text-orange-300 font-medium">
                + Adicionar Cliente
              </button>
            </div>
            <CustomSelect
